...
|------|---------|-------|------|
| `BOOKING_NOT_FOUND` | record not found | 404 | ID does not exist |
| `BOOKING_CODE_ALREADY_EXISTS` | code already exists | 409 | Duplicate detected |

### Validation Errors

| Code | Message | Status| Note |
|------|---------|-------|------|
| `BOOKING_DETAILS_REQUIRED` | details required | 400 | Empty details array |
| `BOOKING_AMOUNT_INCONSISTENT` | amount mismatch | 400 | Sum validation failed |
```
